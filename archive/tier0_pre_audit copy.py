...
def fetch_activities_chunked(
    ...
    # --- Determine mode --------------------------------
    report_type = str(context.get("report_type", "")).lower() if isinstance(context, dict) else ""
    light_mode = report_type == "season"

    if light_mode:
        debug(context, "🧩 Tier-0: season → lightweight activities")
    else:
        debug(context, "🧩 Tier-0: weekly → full activities")
...
def fetch_athlete_profile(headers, from_cache=None, context=None):
    ...
    # -------------------------------------------------
    if from_cache is not None:
        debug(context, "[T0] Using prefetched athlete profile")

        if not isinstance(from_cache, dict):
            raise AuditHalt("❌ Cached athlete profile is not a dict")
    ...
    # -------------------------------------------------
        ...
        debug(context, f"[T0] Fetching athlete profile+sportsettings via OAuth2: {profile_url}")
    ...
    # Reject invalid sources (real data only)
    ...
    # --- Profile metadata from Intervals.icu ---
    merged_profile.update({
    ...
    debug(context, f"[T0] Athlete profile ready — id={athlete['id']} name={athlete.get('name')}")
