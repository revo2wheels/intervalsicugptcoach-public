     ...
     // --- Determine report range automatically ---
      ...
      try {
        ...
        if (reportType === "season") {
          ...
          // --- Build unified payload with full datasets ----------------------------
          const payload = {
            status: "ok",
            ...
            // ✅ include actual data arrays
            activities_light: light,
            activities_full: light, // reuse same for simplicity
            wellness,
      ...
      [light, full, wellness, profile] = await Promise.all([
        ...
        fetch(`${baseWellUrl}?oldest=${getDate(28)}&newest=${getDate(0)}`, {
          headers: { Authorization: authHeader }
        }).then(r => r.json()),
