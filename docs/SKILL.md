---
name: montis-coaching
description: Interpret authorized Montis.icu and Intervals.icu endurance data, generate governed coaching decisions, analyze activities, plan training, and safely manage calendar workflows.
---
...
3. Preserve all fields the user did not ask to change.
4. Update the existing event by ID when available.
5. Clearly state the intended change before calling `calendar_write` or `calendar_delete`, and obtain explicit confirmation whenever the operation may replace, delete, or bulk-modify events.
